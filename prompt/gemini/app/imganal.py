...
def render_image_playground_tab(multimodal_model_pro: GenerativeModel):
    ...
    recommendations, screens, diagrams, equations = st.tabs(
        [
    ...
    with recommendations:
        get_chair_recommendation(multimodal_model_pro)

    with screens:
        get_instructions(multimodal_model_pro)

    with diagrams:
        analyze_er_diagram(multimodal_model_pro)

    with equations:
        analyze_math_equation(multimodal_model_pro)


def get_chair_recommendation(multimodal_model_pro):
...
def get_instructions(multimodal_model_pro):
    oven_screen_uri = "gs://cloud-training/OCBL447/gemini-app/images/oven.jpg"
    oven_screen_url = (
        "https://storage.googleapis.com/" + oven_screen_uri.split("gs://")[1]
    )
    ...
    st.write(
        "Provide instructions for resetting the clock on this appliance in English"
    )
    ...
    with tab1:
        if generate_instructions_description and prompt:
            with st.spinner("Generating instructions using Gemini..."):
                response = get_gemini_pro_vision_response(
                    multimodal_model_pro, [oven_screen_img, prompt]
                )
                st.markdown(response)
                logging.info(response)
    with tab2:
...
def analyze_er_diagram(multimodal_model_pro):
    ...
    er_diag_img_description = st.button(
        "Generate documentation", key="er_diag_img_description"
    )
    with tab1:
...
def analyze_math_equation(multimodal_model_pro):
