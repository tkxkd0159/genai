...
def render_video_playground_tab(multimodal_model_pro: GenerativeModel):
    ...
    video_desc, video_tags, video_highlights, video_geoloc = st.tabs(
        ["Video description", "Video tags", "Video highlights", "Video geolocation"]
    )

    with video_desc:
        generate_video_desc(multimodal_model_pro)

    with video_tags:
        generate_video_tags(multimodal_model_pro)

    with video_highlights:
        generate_video_highlights(multimodal_model_pro)

    with video_geoloc:
        analyze_video_geolocation(multimodal_model_pro)


def analyze_video_geolocation(multimodal_model_pro):
        ...
        if video_geolocation_description and prompt:
            with st.spinner("Generating location information"):
                response = get_gemini_pro_vision_response(
                    multimodal_model_pro, [prompt, video_geolocation_vid]
                )
...
def generate_video_highlights(multimodal_model_pro):
    ...
    with tab1:
        if video_highlights_description and prompt:
            with st.spinner("Generating video highlights"):
                response = get_gemini_pro_vision_response(
                    multimodal_model_pro, [prompt, video_highlights_vid]
                )
                st.markdown(response)
                logging.info(response)
    with tab2:
...
def generate_video_tags(multimodal_model_pro):
    ...
    with tab1:
        if video_tags_desc and prompt:
            with st.spinner("Generating video tags"):
                response = get_gemini_pro_vision_response(
                    multimodal_model_pro, [prompt, video_tags_vid]
                )
                st.markdown(response)
                logging.info(response)
    with tab2:
...
def generate_video_desc(multimodal_model_pro):
    video_desc_uri = (
        "gs://cloud-training/OCBL447/gemini-app/videos/mediterraneansea.mp4"
    )
    video_desc_url = (
        "https://storage.googleapis.com/" + video_desc_uri.split("gs://")[1]
    )
    ...
    tab1, tab2 = st.tabs(["Response", "Prompt"])
    video_desc_description = st.button(
        "Generate video description", key="video_desc_description"
    )
    with tab1:
        if video_desc_description and prompt:
            with st.spinner("Generating video description"):
                response = get_gemini_pro_vision_response(
                    multimodal_model_pro, [prompt, video_desc_vid]
                )
                st.markdown(response)
                logging.info(response)
    with tab2:
